...
from nicewx.config import (
    CATEGORIES,
    OUTPUT_DIR,
    WESTERN_FAIR_NICE_BORDERLINE_MARGIN,
    WESTERN_FAIR_NICE_DIAGNOSTIC_THRESHOLDS,
    WESTERN_FAIR_NICE_DISTRIBUTION_RANGE,
    WESTERN_FAIR_NICE_FOCUS_WINDOWS,
)
from nicewx.validation.western_threshold_sensitivity import build_western_threshold_comparison_frame
...
def category_index_with_fair_nice_threshold(score: float, fair_nice_threshold: float) -> int:
    ...
    if bounded_score >= CATEGORIES[2].lower:
        return 2
    if bounded_score >= fair_nice_threshold:
        return 1
    return 0


def borderline_fair_nice_agreement(
    before_score: float,
    after_score: float,
    before_index: int,
    after_index: int,
    threshold: float = 45.0,
    margin: float = WESTERN_FAIR_NICE_BORDERLINE_MARGIN,
) -> bool:
...
def summarize_fair_nice_audit(frame: pd.DataFrame) -> dict[str, object]:
    """Summarize low-end overlap behavior for the western seam."""

    score_min, score_max = WESTERN_FAIR_NICE_DISTRIBUTION_RANGE
    ...
    fair_nice_flips = frame.loc[frame["production_transition"].isin(["Poor->Fair", "Fair->Poor"])].copy()

    summary: dict[str, object] = {
        "seam_pair": "southwest+rockies",
        "overlap_cell_count": int(len(frame)),
        "distribution_cell_count_35_55": int(len(distribution_frame)),
        "fair_nice_flip_count": int(len(fair_nice_flips)),
    }
    ...
    for lower, upper in WESTERN_FAIR_NICE_FOCUS_WINDOWS:
        label = f"{int(lower)}_{int(upper)}"
        ...
        if not fair_nice_flips.empty:
            flip_any = fair_nice_flips["blended_daily_score_baseline"].between(lower, upper) | fair_nice_flips["blended_daily_score_tuned"].between(lower, upper)
            flip_both = fair_nice_flips["blended_daily_score_baseline"].between(lower, upper) & fair_nice_flips["blended_daily_score_tuned"].between(lower, upper)
            summary[f"fair_nice_flips_in_{label}_any_fraction"] = round(float(flip_any.mean()), 4)
            summary[f"fair_nice_flips_in_{label}_both_fraction"] = round(float(flip_both.mean()), 4)
        else:
            summary[f"fair_nice_flips_in_{label}_any_fraction"] = 0.0
            summary[f"fair_nice_flips_in_{label}_both_fraction"] = 0.0

    return summary


def interpret_fair_nice_results(summary: dict[str, object], alternatives: pd.DataFrame) -> tuple[str, str]:
    """Return a compact interpretation label and recommendation."""

    fair_nice_flip_fraction_42_48 = float(summary["fair_nice_flips_in_42_48_any_fraction"])
    production_row = alternatives.loc[alternatives["mode"] == "production_45"].iloc[0]
    shifted_best = alternatives.loc[alternatives["mode"] != "production_45"].sort_values("fair_nice_flip_count").iloc[0]

    if fair_nice_flip_fraction_42_48 >= 0.65 and float(shifted_best["fair_nice_flip_reduction_fraction"]) < 0.25:
        return (
    ...
    if float(shifted_best["fair_nice_flip_reduction_fraction"]) >= 0.35 and float(shifted_best["total_distribution_shift"]) <= 0.06:
        return (
            "mostly threshold-definition driven",
            "A category-definition adjustment is more justified than more aggregation tuning.",
        )
    if float(production_row["fair_nice_flip_count"]) > 0 and fair_nice_flip_fraction_42_48 < 0.5:
        return (
...
def run_western_fair_nice_audit(
    valid_date: date,
    output_dir: Path,
    ...
    for threshold in WESTERN_FAIR_NICE_DIAGNOSTIC_THRESHOLDS:
        ...
        frame[f"baseline_index_{label}"] = frame["blended_daily_score_baseline"].apply(
            lambda score, threshold=threshold: category_index_with_fair_nice_threshold(score, threshold)
        )
    ...
    frame["borderline_same_tier"] = frame.apply(
        lambda row: borderline_fair_nice_agreement(
    ...
    summary = summarize_fair_nice_audit(frame)
    ...
    production_fair_nice_flip_count = 0
    for threshold in WESTERN_FAIR_NICE_DIAGNOSTIC_THRESHOLDS:
        label = LOW_END_THRESHOLD_LABELS.get(threshold, f"threshold_{threshold:.1f}")
        ...
        fair_nice_flips = frame[transition_col].isin(["Poor->Fair", "Fair->Poor"])

        classified_scores = category_distribution_scores.apply(lambda score, threshold=threshold: category_index_with_fair_nice_threshold(score, threshold))
        distribution = {
        ...
        if label == "production_45":
            ...
            production_fair_nice_flip_count = int(fair_nice_flips.sum())

        alternative_rows.append(
            {
                "mode": label,
                "fair_nice_threshold": threshold,
                "fair_nice_flip_count": int(fair_nice_flips.sum()),
                ...
                "borderline_same_tier_fraction": round(
                    float(
                        (
                            ...
                            | frame.apply(
                                lambda row, threshold=threshold: borderline_fair_nice_agreement(
                                    before_score=row["blended_daily_score_baseline"],
                                    after_score=row["blended_daily_score_tuned"],
    ...
    alternatives["fair_nice_flip_reduction_fraction"] = alternatives["fair_nice_flip_count"].apply(
        lambda count: round(
            (production_fair_nice_flip_count - int(count)) / production_fair_nice_flip_count,
            4,
        )
        if production_fair_nice_flip_count > 0
        else 0.0
    )
    ...
    interpretation, recommendation = interpret_fair_nice_results(summary, alternatives)
    ...
    fair_nice_detail = frame.loc[frame["production_transition"].isin(["Poor->Fair", "Fair->Poor"])].copy()

    summary_path = output_dir / f"nicewx_western_fair_nice_audit_{mesh_profile}_{valid_date:%Y%m%d}.csv"
    alternatives_path = output_dir / f"nicewx_western_fair_nice_alternatives_{mesh_profile}_{valid_date:%Y%m%d}.csv"
    detail_path = output_dir / f"nicewx_western_fair_nice_detail_{mesh_profile}_{valid_date:%Y%m%d}.csv"
    pd.DataFrame([summary]).to_csv(summary_path, index=False)
    alternatives.to_csv(alternatives_path, index=False)
    fair_nice_detail.to_csv(detail_path, index=False)
...
def main() -> None:
    ...
    summary_path, alternatives_path, detail_path = run_western_fair_nice_audit(
        valid_date=valid_date,
    ...
    best_alternative = alternatives.loc[alternatives["mode"] != "production_45"].sort_values("fair_nice_flip_count").iloc[0]
    print(f"Valid date: {valid_date:%Y-%m-%d}")
    print(f"Interpretation: {summary['interpretation']}")
    print(f"Poor/Fair flips: {summary['fair_nice_flip_count']}")
    print(f"Most useful comparison-only alternative: {best_alternative['mode']}")
    print(f"Best alternative Poor/Fair flip reduction: {best_alternative['fair_nice_flip_reduction_fraction']:.4f}")
