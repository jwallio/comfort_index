"""CLI entry point for the Nice Weather Map Generator."""
...
from nicewx.data.loaders import get_loader
...
from nicewx.mapping.mosaic import RegionalDailyRaster, mosaic_regional_rasters
from nicewx.mapping.regions import (
    RegionDefinition,
...
from nicewx.validation.demo_cases import build_demo_case_hourly_breakdown, run_demo_case_validation
from nicewx.validation.inspection import export_point_inspection, inspect_point
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate a daily nice weather map.")
...
def _save_daily_fields(daily: xr.Dataset, valid_date: date, output_dir: Path) -> Path:
    ...
    netcdf_path = output_dir / f"nicewx_daily_fields_{valid_date:%Y%m%d}.nc"
...
def _region_file_prefix(loader_name: str, region_name: str, mesh_profile: str, aggregation_mode: str) -> str:
    ...
    return f"nicewx_region_{region_name}" if loader_name == "mock" else f"nicewx_region_{region_name}_openmeteo{profile_suffix}{aggregation_suffix}"
...
def _mosaic_file_prefix(
    ...
    aggregation_suffix = "" if aggregation_mode == "baseline" else f"_{aggregation_mode}"
    if loader_name == "mock":
        return f"nicewx_mosaic_{ordered_regions}{aggregation_suffix}{method_suffix}"
    return f"nicewx_mosaic_{ordered_regions}_openmeteo{profile_suffix}{aggregation_suffix}{method_suffix}"
    ...
    *,
    ...
    if publish_preset and bool(publish_preset["write_bundle_manifest"]):
        bundle_csv_path, bundle_json_path = write_publish_bundle(
            ...
            product_kind="region",
            product_slug=file_prefix.replace("nicewx_", ""),
...
def run_pipeline(
    ...
    if mosaic_regions:
        ...
        method_suffix = "" if mosaic_blend_method == "taper" and mosaic_target_grid == "adaptive" else f"_{mosaic_blend_method}_{mosaic_target_grid}"
        aggregation_suffix = "" if aggregation_mode == "baseline" else f"_{aggregation_mode}"
        file_prefix = f"nicewx_mosaic_{ordered_regions}_openmeteo{profile_suffix}{aggregation_suffix}{method_suffix}"
        field_path = output_dir / f"{file_prefix}_daily_fields_{valid_date:%Y%m%d}.nc"
        mosaic_daily.to_netcdf(field_path)
        extent = (
            float(mosaic_daily["lon"].min().values),
    ...
    if region is not None:
        ...
        profile_suffix = "" if mesh_profile == "standard" else f"_{mesh_profile}"
        aggregation_suffix = "" if aggregation_mode == "baseline" else f"_{aggregation_mode}"
        file_prefix = f"nicewx_region_{region.name}" if loader_name == "mock" else f"nicewx_region_{region.name}_openmeteo{profile_suffix}{aggregation_suffix}"
        field_path = output_dir / f"{file_prefix}_daily_fields_{valid_date:%Y%m%d}.nc"
    ...
    demo_csv_path = output_dir / f"nicewx_demo_cases_{valid_date:%Y%m%d}.csv"
    demo_hourly_csv_path = output_dir / f"nicewx_demo_case_hourly_{valid_date:%Y%m%d}.csv"
...
def run_pilot_day(
    *,
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    run_slug = _run_slug("nicewx", "pilot_day", loader_name, valid_date.strftime("%Y%m%d"))
...
def main() -> None:
    ...
    if loader_name == "openmeteo":
        if args.mosaic:
            ...
            run_slug = _run_slug("nicewx", "mosaic", *args.mosaic, loader_name, valid_date.strftime("%Y%m%d"))
        elif args.region:
            workflow_name = "region_build"
            label = f"region={args.region};date={valid_date.isoformat()}"
            run_slug = _run_slug("nicewx", "region", args.region, loader_name, valid_date.strftime("%Y%m%d"))
        else:
            workflow_name = "point_build"
            label = f"point={args.lat},{args.lon};date={valid_date.isoformat()}"
            run_slug = _run_slug("nicewx", "point", loader_name, valid_date.strftime("%Y%m%d"))
        reset_openmeteo_request_records()
        try:
