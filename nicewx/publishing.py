...
from nicewx.config import ARCHIVE_SETTINGS, PRODUCT_METADATA, PUBLISH_PRESETS
...
def write_publish_bundle(
    *,
    ...
    csv_path = output_dir / f"nicewx_publish_{product_kind}_{product_slug}_{valid_date:%Y%m%d}_bundle.csv"
    json_path = output_dir / f"nicewx_publish_{product_kind}_{product_slug}_{valid_date:%Y%m%d}_bundle.json"
...
def write_pilot_day_index(
    *,
    ...
    base_name = f"nicewx_pilot_day_{source_name}_{valid_date:%Y%m%d}_index"
    ...
    html_parts = [
        "<html><head><meta charset='utf-8'><title>NiceWX Pilot Day Index</title></head><body>",
...
def write_pilot_day_status_summary(
    *,
    ...
    base_name = f"nicewx_pilot_day_{source_name}_{valid_date:%Y%m%d}_status"
    csv_path = output_dir / f"{base_name}.csv"
    json_path = output_dir / f"{base_name}.json"
...
def write_archive_index(
    *,
    ...
    run_index_paths = sorted(archive_root.glob("**/nicewx_pilot_day_*_index.json"))
    ...
    html_parts = [
        "<html><head><meta charset='utf-8'><title>NiceWX Archive</title></head><body>",
